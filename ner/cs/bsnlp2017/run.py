...
def extract_ner_bio_sents():
    ...
    annt_path = os.path.join('original', 'annotated')
    raw_path = os.path.join('original', 'raw')
    annts = sorted(os.listdir(annt_path))
    annts = list(filter(lambda x: '.txt' in x, annts))
...
def write_ner_bio_file():
    ...
    import random
    all_sents = extract_ner_bio_sents() 
